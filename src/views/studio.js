...
export function studioVisibleNodes(data){const all=studioNodeList(data),filtered=state.studioFilter==="all"?all:all.filter(node=>node.type===state.studioFilter);if(state.studioFilter!=="all")return filtered;const {owner}=buildStudioSceneOwnership(data),collapsed=new Set(state.studioCollapsedScenes||[]);return filtered.filter(node=>{if(node.type==="scene"||node.type==="chapter")return true;const sceneId=owner.get(`${node.type}:${node.id}`);if(!sceneId)return true;return !collapsed.has(sceneId)})}

export function studioFilterButton(value,label){return `<button class="filter-tab ${state.studioFilter===value?"active":""}" data-action="studio-filter" data-filter="${value}">${label}</button>`}

export function studioCompactSelection(data){const selected=state.studioSelectedNode;if(!selected)return `<div class="canvas-tip">场景卡片上的 ▸ 可折叠/展开其下线索、调查点与物品；点「自动排布」优先使用「场景分支」避免重叠。拖动画布空白处可平移。</div>`;return `<div class="compact-selection"><span>当前节点：${studioNodeName(data,selected.type,selected.id)} · ${studioNodeAnchors(studioNodeRecord(data,selected.type,selected.id)).length} 个连接点</span><div class="row"><button class="text-btn" data-action="studio-add-anchor">＋ 连接点</button><button class="text-btn" data-action="studio-toggle-anchor-edit">${state.studioAnchorEditing?"完成调整":"调整连接点"}</button><button class="text-btn" data-action="studio-connect-node">＋ 创建连线</button><button class="text-btn danger-text" data-action="studio-delete-node">删除节点</button></div></div>`}
...
export function setStudioNodePosition(type,id,position){const node=studioNodeRecord(state.cloudStudio,type,id);if(node)node.metadata={...(node.metadata||{}),graphPosition:position}}

export function setStudioNodeAnchors(type,id,anchors){const node=studioNodeRecord(state.cloudStudio,type,id);if(node)node.metadata={...(node.metadata||{}),graphAnchors:anchors}}
...
export async function autoLayoutStudio(mode = state.studioLayoutMode || "scene-tree"){
 try{
