...
export function studioNodeEditPanel(data,selected){
 ...
 if(selected.type==="item"){
  const { cloudAssets } = assetStore.get();
  const assets=[{id:"",name:"不关联附件"},...(cloudAssets||[]).map(asset=>({id:asset.id,name:asset.original_filename}))];
  const action=(meta.itemActions||[])[0]||{};
  const combineItems=[{id:"",name:"不需要组合物"},...(data.items||[]).filter(item=>item.id!==record.id)];
  return `<div class="studio-edit-panel"><p class="section-kicker">编辑物品</p>${studioEditField("物品名称","name","input",record.name)}${studioEditField("物品描述","publicText","textarea",record.public_text||"")}<label class="studio-check-row"><input type="checkbox" data-studio-edit-boolean="unique" ${meta.unique?"checked":""}> 是否唯一（同一角色不可重复获得）</label><label class="studio-check-row"><input type="checkbox" data-studio-edit-boolean="consumable" ${meta.consumable?"checked":""}> 是否可消耗（使用后消失）</label><div class="studio-item-action-contract"><p class="section-kicker">玩家动作契约</p>${studioEditSelect("主要动作","actionKind",[{id:"",name:"仅展示，不可主动使用"},{id:"use",name:"使用"},{id:"consume",name:"消耗"},{id:"combine",name:"组合"}],action.kind||"")}${studioEditField("玩家按钮文案","actionLabel","input",action.label||"")}${studioEditSelect("作用目标","actionTargetType",[{id:"none",name:"无需选择目标"},{id:"role",name:"选择房间角色"},{id:"location",name:"选择已揭示地点"}],action.targetType||"none")}${studioEditSelect("允许组合的物品","combineItemId",combineItems,action.combineWithItemIds?.[0]||"")}${studioEditField("消耗本物品数量","consumeQuantity","input",action.consumeQuantity??0)}${studioEditField("消耗组合物数量","combineConsumeQuantity","input",action.combineConsumeQuantity??0)}${studioEditField("动作完成提示","actionResultText","textarea",action.resultText||"")}<label class="studio-check-row"><input type="checkbox" data-studio-edit-boolean="requiresHostConfirmation" ${action.requiresHostConfirmation?"checked":""}> 提交后由主持人确认，再扣减库存并公布结果</label></div>${studioEditSelect("关联资产","assetId",assets,meta.assetId||"")}${studioEditField("主持备注","hostText","textarea",record.host_text||"")}<button class="primary-btn full-btn" data-action="studio-save-node">保存物品</button></div>`;
 }
...
export async function saveSelectedStudioNode(){
 ...
 try{
  if(studioSelectedNode.type==="chapter"){
   ...
   const itemActions=values.actionKind?[{key:"primary",label:values.actionLabel||"使用",kind:values.actionKind,targetType:values.actionTargetType||"none",requiresHostConfirmation:Boolean(values.requiresHostConfirmation),consumeQuantity:Number(values.consumeQuantity)||0,combineConsumeQuantity:Number(values.combineConsumeQuantity)||0,combineWithItemIds:values.combineItemId?[values.combineItemId]:[],resultText:values.actionResultText||""}]:[];
   await zhimuApi.updateItem(studioSelectedNode.id,{name:values.name,publicText:values.publicText,hostText:values.hostText,unique:Boolean(values.unique),consumable:Boolean(values.consumable),assetId:values.assetId||null,itemActions});
  }else{
