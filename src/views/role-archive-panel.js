...
export function renderRoleArchiveFields(role, archive = {}, { readOnly = false } = {}) {
  ...
  return `<div class="role-archive-panel" data-role-archive="${escapeHtml(role.id)}">
    ...
    <div class="role-archive-grid">
      ...
      <label class="cockpit-field compact"><span>隐藏身份</span><textarea data-archive-field="hiddenIdentity" data-role-id="${escapeHtml(role.id)}" rows="2"${readonly}>${escapeHtml(archive.hiddenIdentity || "")}</textarea></label>
