  ...
  function feedbackRows(items = []) {
    ...
    return items.map((item) => {
      ...
      return `<div class="checkpoint-row feedback-row">
        <strong>${escapeHtml(label[item.kind] || item.kind || "反馈")} · ${escapeHtml(item.subject || "")}</strong>
        <p>${escapeHtml(item.status || "new")} · ${escapeHtml(reporter)} · ${formatTime(item.created_at)}</p>
        <p>${escapeHtml(item.body || "")}</p>
        ${item.page_url ? `<p class="muted-note">${escapeHtml(item.page_url)}</p>` : ""}
        <div class="row"><button class="secondary-btn" data-action="ops-feedback-status" data-feedback-id="${escapeHtml(item.id)}" data-feedback-status="${escapeHtml(status)}">${escapeHtml(button)}</button></div>
      </div>`;
...
export function ops() {
    ...
    <article class="card"><div class="section-head"><div><h3>用户反馈</h3><p>${feedbackTotal} 条记录 · ${feedbackNewCount} 条待看。</p></div><span class="status-chip ${feedbackNewCount ? "testing" : "published"}">${feedbackNewCount ? "ACTION" : "CLEAR"}</span></div><div class="host-detail-list">${feedbackRows(feedback)}</div></article>
