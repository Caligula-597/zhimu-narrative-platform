...
function renderInspirationWall(cockpit) {
  ...
  const cards = sparks.length
    ? sparks.map((s) => `<article class="spark-card">
  ...
  const tagOpts = SPARK_TAGS.map((t) => `<option value="${escapeHtml(t)}">${escapeHtml(t)}</option>`).join("");
...
function renderContentOverviewPanel(ctx) {
  const rows = buildContentOverview(ctx).map(([label, value]) => `
    <article class="overview-stat"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></article>`).join("");
...
export function renderConceptCanvas(ctx, cockpit, findItemLink) {
  ...
  if (cockpit.activeCanvas === "selling") {
    return `<section class="cockpit-panel">
      ...
      <div class="selling-slots">${cockpit.sellingPoints.map((point, index) => `
        <article class="selling-slot"><span>卖点 ${index + 1}</span>
          <textarea data-cockpit-field="selling-${index}" rows="3">${escapeHtml(point)}</textarea></article>`).join("")}
      </div></section>`;
  }
...
function relationAddForm(roles) {
  ...
  return renderQuickForm("新增关系边", `
    ...
    <input class="field" data-cockpit-rel="strength" type="number" min="-10" max="10" placeholder="强度 -10～10（可选）">`, "cockpit-add-relationship");
}

function chapterAddForm() {
  return renderQuickForm("新增公共章节", `
    <input class="field" data-cockpit-chapter="title" placeholder="章节标题，如：第一幕 · 暴风雪夜">
    <textarea class="field" data-cockpit-chapter="summary" rows="2" placeholder="本章摘要（可选）"></textarea>`, "cockpit-add-chapter");
}
...
export function renderCharactersCanvas(ctx, cockpit, findItemLink) {
  ...
  const cards = roles.length
    ? roles.map((role) => {
        const roleSections = sections.filter((s) => s.role_slot_id === role.id);
        const count = roleSections.length;
        const pub = roleSections.filter((s) => s.publication_status !== "draft").length;
...
export function renderFlowCanvas(ctx, cockpit, findItemLink) {
  ...
  if (cockpit.activeCanvas === "matrix") {
    ...
    return `<section class="cockpit-panel"><div class="panel-heading"><div><p>线索 × 角色</p><h3>绿格 = Segment 已标注应发线索</h3></div>${linkButton({ view: "structure", label: "编辑 clueGrants" })}</div>
      ...
      <tbody>${clues.map((clue) => `<tr><th>${escapeHtml(clue.name)}</th>${roles.map((r) => {
  ...
  if (cockpit.activeCanvas === "sandbox") {
    return `<section class="cockpit-panel"><div class="panel-heading"><div><p>主持预演</p><h3>${counts.rooms || 0} 个运行房</h3></div></div>
      <div class="row">${linkButton({ action: "world-rooms", label: "管理运行房" }, "primary-btn")}${linkButton({ action: "open-host-console", label: "打开主持端" })}</div></section>`;
  }
  const segRows = segments.length
    ? segments.map((s) => `<button type="button" class="host-current-item cockpit-segment-pick ${s.id === cockpit.selectedSegmentId ? "active" : ""}" data-cockpit-segment-id="${escapeHtml(s.id)}">
...
export function renderManuscriptCanvas(ctx, cockpit) {
  ...
  if (cockpit.activeCanvas === "cards") {
    ...
    const cards = clues.length
      ? clues.slice(0, 12).map((c) => `<article class="prop-card"><div class="prop-image">线索</div><strong>${escapeHtml(c.name)}</strong><small>${escapeHtml(c.public_text?.slice(0, 40) || "待补充说明")}</small></article>`).join("")
...
function checkLevelLabel(level) {
  return ({ error: "必填缺失", warning: "待确认", info: "通过", ok: "通过" })[level] || level;
}
...
function renderCopilotAnalysis(cockpit) {
  ...
  if (analysis?.nodes?.length) {
    const nodeLines = analysis.nodes.slice(0, 12).map((n) => `<li><code>${escapeHtml(n.type || "node")}</code> ${escapeHtml(n.label || n.name || n.text || "")}</li>`).join("");
    parts.push(`<p class="muted-note">识别 ${analysis.nodes.length} 个结构节点</p><ul class="cockpit-hint-list">${nodeLines}</ul>`);
  }
  if (aiNote) parts.push(`<article class="card-lite"><strong>AI 输出</strong><p>${escapeHtml(aiNote)}</p></article>`);
  return parts.length ? parts.join("") : `<p class="muted-note">可选：结构识别（规则解析）或 AI 续写（作者主动发起，需 LLM）。</p>`;
}
...
export function renderAssistant(stage, ctx, cockpit) {
  ...
  const cards = facts.map(([k, v]) => `<article class="copilot-fact"><strong>${escapeHtml(k)}</strong><p>${escapeHtml(v)}</p></article>`).join("");
