...
export function storyManuscriptModalHtml({ bodyHtml, statusHtml, readOnly = false }) {
  bodyHtml = unwrapHtmlFragment(bodyHtml, "manuscript body");
  statusHtml = unwrapHtmlFragment(statusHtml, "manuscript status");
  return `<h2>完整剧情母稿</h2><p class="wizard-intro">${readOnly ? "只读审稿模式：你可以核对完整母稿并在审稿台提交意见，不能修改或导出正文。" : "这是创作者维护的全局剧情文稿，不会替代每位角色的私人剧本。你可以从剧情编排生成一份规范化母稿，也可以把编辑后的母稿拆分成场景、调查点、线索与连接线。"}</p>${readOnly ? "" : '<div class="assistant-guide"><b>双向同步边界</b><span>“从编排台生成母稿”会覆盖下方文本；“拆分母稿写回编排台”会重建此前由母稿生成的节点，不会删除你手工建立的节点。</span></div>'}<textarea class="field manuscript-draft" rows="20" data-story-manuscript${readOnly ? " readonly" : ""}>${bodyHtml}</textarea><div class="manuscript-meta" data-manuscript-meta>${statusHtml}</div><div class="modal-actions"><button class="secondary-btn" data-close>关闭</button>${readOnly ? "" : '<button class="secondary-btn" data-manuscript-save>仅保存母稿</button><button class="secondary-btn" data-manuscript-from-graph>从编排台生成母稿</button><button class="primary-btn" data-manuscript-to-graph>拆分母稿写回编排台</button>'}</div>`;
}
...
export function documentParserModalHtml(roleOptionsHtml) {
  ...
  return `<h2>稿件解析与结构化导入</h2><p class="wizard-intro">支持 TXT / Markdown / DOCX / PDF / 图片与飞书云文档。系统会先识别角色、幕、场景、线索和秘密，只有确认预览后才写入；结构化内容默认保存为草稿，线索默认仅主持可见。</p><div class="form-group"><label>创作类型</label><select class="field" data-document-creation-type><option value="murder_mystery">剧本杀 · 角色本 / 公共幕 / 线索</option><option value="tabletop_rpg">跑团 · PC / 章节 / HO / KP 信息</option><option value="interactive_story">互动叙事 · 角色 / 章节 / 信息卡</option></select><label>稿件来源</label><select class="field" data-document-source><option value="file">本地文件</option><option value="feishu">飞书云文档</option></select><div data-document-file-fields><label>选择文档</label><input class="field" type="file" accept=".txt,.md,.markdown,.docx,.pdf,.jpg,.jpeg,.png,.webp" data-document-file></div><div data-document-feishu-fields hidden><label>飞书文档链接</label><input class="field" type="url" inputmode="url" placeholder="https://...feishu.cn/docx/..." data-document-feishu-url><p class="muted-note">需在飞书文档的“添加文档应用”中授予只读权限；平台不会保存飞书访问凭据。</p></div><label>写入目标</label><select class="field" data-document-target><option value="structured">智能结构导入 · 新建草稿角色/幕/场景/线索/秘密</option><option value="manuscript">完整剧情母稿</option>${roleOptionsHtml}</select><label class="checkbox-line" style="margin-top:10px"><input type="checkbox" data-document-allow-ocr> 图片型 PDF 尝试 OCR 为文字（较慢，需复核）</label><label>PDF 图片导入布局（仅图片模式）</label><select class="field" data-document-page-layout><option value="single_section">整份 PDF 合并为一个分幕</option><option value="one_section_per_page">每页单独一个分幕</option></select><aside class="tutorial-tip"><strong>版权与保密</strong><span>未授权稿件不得导入。私有稿件不会自动公开或作为平台训练数据；只有你主动调用 AI 时，相关上下文才可能发送给所选模型供应商。可删除在线稿件并导出完整结构化归档，附件文件需另行下载；受限备份按保留周期轮转清除。</span></aside><label class="checkbox-line"><input type="checkbox" data-document-rights-confirmed> 我确认拥有该稿件或已取得处理与导入授权</label></div><div data-document-preview></div><div class="modal-actions"><button class="secondary-btn" data-close>取消</button><button class="secondary-btn" data-document-parse>解析预览</button><button class="primary-btn" data-document-import disabled>确认导入</button></div>`;
}
