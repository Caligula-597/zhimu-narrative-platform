...
function writer(){
 ...
 if(!data)return U.creatorWorkspaceEmpty?.({title:"剧本杀创作中心",kicker:"SCRIPTED MYSTERY CREATOR",intro:"为每位玩家编写私人分幕，并控制公共章节的测试与发布节奏。尚未选择剧本时，可先浏览公开库或创建新世界。",guideTitle:"创作台会提供什么",guideItems:[{label:"角色稿",title:"私人分幕正文",text:"每位玩家只看到自己的章节与秘密。",bullets:["按角色席位分组管理分幕","支持 Markdown 与版本快照","发布状态：草稿 / 测试中 / 已发布"]},{label:"章节",title:"公共章节与解锁",text:"控制玩家何时能看到下一章公共信息。",bullets:["主持人确认或自动解锁","与规则引擎联动"]},{label:"工具",title:"导入导出与 AI 辅助",text:"连接编排台与运行房之前的内容生产。",bullets:["内容包导入导出","DeepSeek 结构提案（需配置）"]}]})||`<section class="card"><h3>尚未选择剧本</h3><p><button class="primary-btn" data-action="open-catalog">浏览公开剧本库</button></p></section>`;
...
async function deleteCreatorSnapshot(versionId){try{await zhimuApi.deleteContentVersion(versionId);await loadCloudData();showToast("创作版本记录已删除")}catch(error){showToast(error.message)}}
  ...
  viewExports.placeholderModule = placeholderModule;
