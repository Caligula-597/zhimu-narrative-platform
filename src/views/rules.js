...
export function openRuleEditor(ruleId=""){
 ...
 const renderVisual=()=>{setHtml(modal.querySelector("[data-rule-visual-panel]"),window.zhimuRuleVisual.renderVisualPanel(visualModel,studio,escapeHtml));wireRuleVisualPanel()};
 ...
 const showRuleErrors=(errors=[])=>{const box=modal.querySelector("[data-rule-errors]");if(!errors.length){setHtml(box,"");box.classList.remove("show");return}box.classList.add("show");setHtml(box,`<strong>请修正以下问题：</strong><ul>${errors.map(item=>`<li>${escapeHtml(item.message)}</li>`).join("")}</ul>`)};
 ...
 modal.querySelector("[data-rule-submit]").onclick=async()=>{
  try{
   ...
   const payload={roomId:values.roomId||null,name:values.name,mode:values.mode,priority:Number(values.priority)||100,enabled:modal.querySelector("[data-rule-enabled]").checked,conditions,actions,metadata:{...(rule?.metadata||{}),...(values.storyPurpose?{storyPurpose:values.storyPurpose}:{storyPurpose:null})}};
