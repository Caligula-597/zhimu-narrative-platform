...
function recapStoryNarrative(snapshot){
 ...
 return `${revelation}<section class="recap-section"><h4>全剧脉络 · 上帝视角</h4><p>${escapeHtml(opening.summary||"")}</p>${cast?`<ul>${cast}</ul>`:""}<div class="recap-chapter-stack">${chapters}</div><article class="recap-chapter-act"><div class="recap-chapter-head"><strong>${escapeHtml(epilogue.title||"结局与余波")}</strong><span class="muted-note">${escapeHtml(epilogue.summary||"")}</span></div>${epilogueBeats}</article></section>`;
}
