...
function directorRiskItem(item){
 ...
 const button=item.action?`<button type="button"${actionAttr}>${escapeHtml(item.button||"处理")} →</button>`:"";
 return `<div class="risk-item ${cls}"><span class="risk-icon">${icon}</span><div><b>${escapeHtml(item.title)}</b><p>${escapeHtml(item.detail)}</p></div>${button}</div>`;
}
...
export function director(){
 ...
 const directorRisks=[];
 if(hostPlayersError){directorRisks.push({level:"error",title:"玩家运行状态加载失败",detail:hostPlayersError,action:"refresh-host-players",button:"重试"});}
 if(!roomEventsConnected){directorRisks.push({level:"warning",title:"实时推送未连接",detail:"当前依赖定时轮询，待确认事件与玩家进度可能有延迟。",action:"refresh-host-room",button:"刷新连接"});}
 if(pendingEvents.length>5){directorRisks.push({level:"warning",title:"待确认事件积压",detail:`${pendingEvents.length} 条事件等待处理，可能影响玩家体验。`,action:"refresh-host-events",button:"查看待办"});}
 if(stuckCount>0){directorRisks.push({level:"warning",title:`${stuckCount} 名玩家疑似卡关`,detail:"已识别具体停滞原因，可逐人查看并发送针对性提醒。",action:"host-stuck-intervene",button:"处理卡点"});}
 if(!rules.length){directorRisks.push({level:"warning",title:"当前房间无启用规则",detail:"自动化规则（发线索、开场景、主持确认）尚未配置，全部依赖手动操作。",action:"rules-preview",button:"查看规则"});}
 const directorRiskErrorCount=directorRisks.filter(r=>r.level==="error").length;
 const directorRiskWarningCount=directorRisks.filter(r=>r.level==="warning").length;
 const directorHasRisks=directorRisks.length>0;
 const roomReady=!hostPlayersError&&roomEventsConnected&&joinedCount>0;
 const roomReadyLabel=roomReady?"运行就绪":!joinedCount?"等待玩家入房":!roomEventsConnected?"连接中":"初始化中";
 ...
 <section class="director-console ${escapeHtml(S.surface?.("host")?.className || "")}">
  <article class="director-console-main">
   <div class="section-head"><div><p class="section-kicker">RUN CONTROL</p><h3>运行控制台</h3><p>先看房间状态、玩家推进和人工待办，再处理下方细表。</p></div><div class="row"><span class="status-chip ${roomReadyTone}">${roomReadyLabel}</span><button class="secondary-btn" data-action="refresh-host-room">刷新现场</button></div></div>
 ...
 <section class="risk-console">
  <div class="section-head">
   ...
   <div class="risk-summary">
    ${directorRiskErrorCount?`<span class="risk-count risk-error">${directorRiskErrorCount} 阻塞</span>`:""}
    ${directorRiskWarningCount?`<span class="risk-count risk-warning">${directorRiskWarningCount} 警告</span>`:""}
    ${!directorHasRisks?`<span class="risk-count risk-ok">✓ 无风险</span>`:""}
   </div>
  </div>
  <div class="risk-list">${directorHasRisks?directorRisks.map(directorRiskItem).join(""):`<div class="empty-state">房间运行正常，无阻塞或警告项。</div>`}</div>
 </section>
...
export async function openHostClueNote(clueId,roleSlotId){
 ...
 modal.className="modal";setHtml(modal,hostClueNoteModalHtml({playerName:player.player_display_name||player.role_name,clueName:clue.name,existing}));
...
export function openHostGrantClueModal(){
 ...
 modal.className="modal";setHtml(modal,hostGrantClueModalHtml({clueSelectHtml:studioSelect("线索","grantClue",clues.map(clue=>({id:clue.id,name:clue.name}))),memberRowsHtml:players.map(player=>`<label><input type="checkbox" data-grant-role value="${player.role_slot_id}"> <span><b>${escapeHtml(player.player_display_name||"玩家")}</b> · ${escapeHtml(player.role_name)}</span></label>`).join(""),messageFieldHtml:studioField("日志说明","grantMessage","input","主持人手动发放线索")}));
...
export function openDelayHostEventModal(eventId){
 ...
 modal.className="modal";setHtml(modal,delayHostEventModalHtml(event.title));
...
export function openHostGrantItemModal(){
 ...
 modal.className="modal";setHtml(modal,hostGrantItemModalHtml({roleSelectHtml:studioSelect("目标角色","grantRole",players.map(player=>({id:player.role_slot_id,name:`${player.player_display_name||"玩家"} · ${player.role_name}`}))),itemSelectHtml:studioSelect("物品","grantItem",items.map(item=>({id:item.id,name:item.name}))),quantityFieldHtml:studioField("数量","grantQuantity","input","1"),messageFieldHtml:studioField("日志说明","grantMessage","input","主持人手动发放物品")}));
...
export function openHostUnlockSectionModal(options = {}){
 ...
 const firstRoleId=options.roleSlotId||players[0].role_slot_id;
 modal.className="modal";setHtml(modal,hostUnlockSectionModalHtml({roleSelectHtml:studioSelect("目标角色","unlockRole",players.map(player=>({id:player.role_slot_id,name:`${player.player_display_name||"玩家"} · ${player.role_name}`}))),sectionSelectHtml:studioSelect("分幕","unlockSection",sections.filter(section=>section.role_slot_id===firstRoleId).map(section=>({id:section.id,name:`${section.sequence}. ${section.title}`}))),messageFieldHtml:studioField("日志说明","unlockMessage","input","主持人手动解锁分幕")}));
 modalBackdrop.classList.add("show");modal.querySelector("[data-close]").onclick=closeModal;const roleSelect=modal.querySelector('[data-studio-field="unlockRole"]'),sectionSelect=modal.querySelector('[data-studio-field="unlockSection"]');if(roleSelect)roleSelect.value=firstRoleId;const refreshSections=()=>{const roleId=roleSelect.value;const options=sections.filter(section=>section.role_slot_id===roleId).map(section=>({id:section.id,name:`${section.sequence}. ${section.title}`}));setHtml(sectionSelect,options.length?studioOptionsHtml(options,""):'<option value="">该角色尚无分幕</option>')};roleSelect.onchange=refreshSections;refreshSections();modal.querySelector("[data-host-unlock-submit]").onclick=async()=>{try{const values=studioValues();await zhimuApi.hostUnlockSection({roleSlotId:values.unlockRole,scriptSectionId:values.unlockSection,message:values.unlockMessage});closeModal();await refreshHostRoom();showToast("分幕已解锁")}catch(error){showError(error)}};
}
...
export function openHostUnlockSceneModal(){
 ...
 modal.className="modal";setHtml(modal,hostUnlockSceneModalHtml(studioSelect("场景","unlockScene",scenes.map(scene=>({id:scene.id,name:scene.name})))));
...
export function openHostLogModal(){
 const { cloudHostPlayers } = roomStore.get();
 modal.className="modal";setHtml(modal,hostLogModalHtml({roleSelectHtml:studioSelect("关联角色","logRole",[{id:"",name:"不指定角色"},...(cloudHostPlayers||[]).filter(player=>player.joined).map(player=>({id:player.role_slot_id,name:`${player.player_display_name||"玩家"} · ${player.role_name}`}))]),messageFieldHtml:studioField("日志内容","logMessage","textarea","例如：提醒林夏继续阅读序章")}));
...
export function openHostNudgeWaitingModal(roleSlotId="", mode="waiting"){
 const waitingIds=pendingEventRoleIds();
 const { cloudHostPlayers } = roomStore.get();
 const allPlayers=(cloudHostPlayers||[]).filter((player)=>player.joined);
 const isStuck=mode==="stuck";
 const players=isStuck?allPlayers.filter((player)=>player.maybe_stuck&&(!roleSlotId||String(player.role_slot_id)===String(roleSlotId))):allPlayers.filter((player)=>!waitingIds.size||waitingIds.has(String(player.role_slot_id)));
 ...
 modal.className="modal";setHtml(modal,hostNudgeModalHtml({title:isStuck?"帮助卡关玩家":"提醒等待中的玩家",intro:isStuck?"系统已根据玩家进度生成建议话术，发送前可以修改。":"消息会通过实时推送送达 play 端与玩家视角，不会发送站外私信。",messageHtml:escapeHtml(defaultMessage),memberRowsHtml:players.map((player)=>`<label><input type="checkbox" data-nudge-role value="${player.role_slot_id}" checked> <span><b>${escapeHtml(player.player_display_name||"玩家")}</b> · ${escapeHtml(player.role_name)}${player.stuck_label?` · ${escapeHtml(player.stuck_label)}`:""}</span></label>`).join("")}));
...
export function openHostMiniGameModal(){
 ...
 setHtml(modal,hostMiniGameModalHtml());
