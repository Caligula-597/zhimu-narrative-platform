...
本目录是基于 2026-06-28 完整代码通读 + 主应用 dev 预览（`http://localhost:4173/`）得出的产品、架构、代码、性能、安全、宣传六位一体优化计划。
...
- **主应用**（根 `src/`）：仍是 `window.* + IIFE` 伪 ES 模块，`zhimuState` 60+ 属性平铺，`api/client.js` 599 行单体，是当前最大的架构债。
- **共享层**：只有 `shared/tokens.css`，三端 API/state/security/styles 重复实现。
