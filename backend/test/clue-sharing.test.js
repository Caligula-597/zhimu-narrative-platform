...
async function queryFixturePeerRoleId(ownerRoleId) {
  ...
  await query(
    ...
    [fixtureRoomId, inserted.rows[0].id]
  );
...
test("player can share owned clue to room and others can see it", async (context) => {
  ...
  const note = await app.inject({
    method: "PATCH",
    url: `/api/rooms/${fixtureRoomId}/clues/${clueId}/player-note`,
    headers: { "x-user-id": playerUserId },
  ...
  const matrix = await app.inject({
    method: "GET",
    url: `/api/rooms/${fixtureRoomId}/host/clue-matrix`,
    headers: { "x-user-id": hostUserId }
  });
...
test("host can delay pending event and wake when due", async (context) => {
  ...
  const inserted = await query(
    `INSERT INTO pending_host_events (room_id, event_key, title, description, actions, status)
     ...
     RETURNING id`,
    [fixtureRoomId, `delay-test-${Date.now()}`]
  );
  const eventId = inserted.rows[0].id;

  const delay = await app.inject({
    method: "POST",
    url: `/api/rooms/${fixtureRoomId}/host-events/${eventId}/delay`,
