...
import { hostUserId, playerUserId, fogRoomId } from "./helpers/fixture-ids.js";
...
test("investigate endpoint is idempotent with Idempotency-Key", async (context) => {
  ...
  const worldId = (await query(`SELECT world_id FROM rooms WHERE id = $1`, [fogRoomId])).rows[0].world_id;
  ...
  await query(
    ...
    [fogRoomId, scene.rows[0].id]
  );
  context.after(async () => {
    await query(`DELETE FROM investigation_records WHERE room_id = $1 AND investigation_point_id = $2`, [fogRoomId, point.rows[0].id]);
    await query(`DELETE FROM investigation_points WHERE id = $1`, [point.rows[0].id]);
  });

  const key = `investigate-${Date.now()}`;
  const url = `/api/rooms/${fogRoomId}/investigation-points/${point.rows[0].id}/investigate`;
  ...
  const records = await query(
    ...
    [fogRoomId, point.rows[0].id, roleSlotId]
  );
...
test("host event dismiss is idempotent with Idempotency-Key", async (context) => {
  ...
  const inserted = await query(
    `INSERT INTO pending_host_events (room_id, event_key, title, description, actions, status)
     ...
     RETURNING id`,
    [fogRoomId, `idempotent-dismiss-${Date.now()}`]
  );
  ...
  const first = await app.inject({
    method: "POST",
    url: `/api/rooms/${fogRoomId}/host-events/${eventId}/dismiss`,
