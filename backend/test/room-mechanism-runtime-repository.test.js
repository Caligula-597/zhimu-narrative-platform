...
function assertingClient(row = storedRow()) {
  ...
  return {
    ...
    async query(sql, params) {
      ...
      assert.equal(highestPlaceholder, params.length, "SQL placeholders must match the supplied parameter list");
      return { rows: [sql.includes("room_mechanism_action_log") ? { id: "action-1", created_at: row.updated_at } : row] };
    }
  };
}
...
test("room mechanism state writes keep SQL parameters and persisted runtime fields aligned", async () => {
  ...
  const inserted = await insertRoomMechanismState(insertClient, { ...binding, runtime: runtime() });
  ...
  assert.equal(insertClient.calls[0].params[12], JSON.stringify({ "state-auth": "unknown" }));
  ...
  assert.equal(updateClient.calls[0].params.length, 15);
});
...
test("room mechanism action log persists the revision edge and structured changes", async () => {
  ...
  const result = await appendRoomMechanismAction(client, {
    ...
    optionKey: "accept",
    changes: [{ targetType: "state", targetKey: "state-auth", after: "accepted" }],
