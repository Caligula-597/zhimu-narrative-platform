...
function fixtureSnapshot() {
  return {
    world: {
      ...
      settings: {
        ...
        mechanismDesign: {
          ...
          title: "临时调度许可",
          recurringAction: "消耗一张许可，核验一份调度记录",
          conflictReason: "公开调度记录会同步暴露持有者身份",
        },
    ...
    roles: [{ id: "role-1", name: "沈青", public_profile: "列车报务员" }],
    roleArchives: [
      {
        id: "archive-1",
        role_slot_id: "role-1",
        external_goal: "确认列车真正目的地",
      },
    ],
    chapters: [
      {
...
function generatedCandidate() {
  return {
    ...
    centralConflict: {
      text: "公开身份有助于验证命令，却会暴露各自任务。",
      sourceRefs: ["brief:selling-points"],
    },
    playerPremise: {
      text: "只有六名乘客拥有互相冲突但可拼合的调度信息。",
      sourceRefs: ["world:summary"],
    },
    mechanismLoop: {
      text: "玩家交换一次身份凭证，换取一次调度记录核验。",
      sourceRefs: ["truth:truth-1"],
    },
    truthAndReversal: {
      text: "所谓临时军令来自被替换的时刻表。",
      sourceRefs: ["truth:truth-1", "unknown:source"],
    },
    roleFunctions: [
      {
        roleId: "role-1",
    ...
    chapterArc: [1, 2, 3].map((sequence) => ({
      ...
      title: `第${sequence}章`,
      cause:
        sequence === 1 ? "列车异常停靠" : `第${sequence - 1}章留下的权限变化`,
...
test("story spine source catalog keeps stable authored references", () => {
  ...
  const mechanism = catalog.find(
    (item) => item.key === "mechanism-design:author",
  );
  assert.equal(mechanism?.type, "mechanism_design");
  assert.match(mechanism?.text || "", /临时调度许可/);
  assert.match(mechanism?.text || "", /作者已确认/);
});

test("story spine assembly preserves author-confirmed blocks and filters fake sources", async () => {
  const snapshot = fixtureSnapshot();
  const result = await assembleStorySpine(
    snapshot,
    {},
    {
      requestId: "request-1",
  ...
  assert.deepEqual(result.storySpine.truthAndReversal.sourceRefs, [
    "truth:truth-1",
  ]);
...
test("authors can confirm one spine section without canonizing every AI draft", () => {
  ...
  assert.deepEqual(
    diff.changedSections.map((item) => item.key),
    ["overview"],
  );
});
