...
const player = { actorId: "user-1", memberType: "player", roleSlotId: "role-1" };

test("hosts receive the complete room event payload", () => {
  const event = { type: "room.host_nudge", message: "secret", roleSlotIds: ["role-2"] };
...
test("private nudges, actions and voice activity enforce their explicit audience", () => {
  assert.equal(projectRoomEventForAudience({
    type: "room.host_nudge", message: "only role 2", roleSlotIds: ["role-2"]
  }, player).event, null);
  assert.equal(projectRoomEventForAudience({
    type: "room.private_action_updated", actionId: "a1", status: "seen", roleSlotIds: ["role-2"]
  }, player).event, null);
  assert.equal(projectRoomEventForAudience({
  ...
  assert.ok(projectRoomEventForAudience({
    type: "room.voice_message_created",
    voiceRoomId: "voice-1",
    messageId: "message-1",
    audience: "restricted",
...
test("hidden journal events become cursor-only heartbeats", () => {
  const projected = projectRoomEventEnvelope({
    id: 42,
    payload: JSON.stringify({ type: "room.item_granted", roleSlotId: "role-2", itemName: "secret" })
  }, player);
...
test("a kicked player receives the terminal event and is disconnected", () => {
  const target = projectRoomEventForAudience({
    type: "room.player_kicked", userId: "user-1", roleSlotId: "role-1", roleName: "A"
  }, player);
  assert.ok(target.event);
  assert.equal(target.disconnectAfter, true);

  const observer = projectRoomEventForAudience({
    type: "room.player_kicked", userId: "user-2", roleSlotId: "role-2", roleName: "B"
  }, player);
...
test("new or malformed event types are host-only by default", () => {
  assert.equal(projectRoomEventForAudience({ type: "room.future_secret", value: "x" }, player).event, null);
