...
test("GET /api/platform/public-rooms lists only public_listing rooms", async () => {
  const app = await createApp();
  const room = await query(
    `INSERT INTO rooms (world_id, host_user_id, name, invite_code, status, public_listing)
     VALUES ($1, $2, $3, $4, 'testing', true)
     RETURNING id, invite_code`,
    [fixtureWorldId, hostUserId, `公开大厅测试-${Date.now()}`, `LOBBY-${Date.now().toString(36).toUpperCase()}`]
  );
...
test("PATCH room listing toggles public_listing", async () => {
  const app = await createApp();
