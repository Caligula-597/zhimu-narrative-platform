...
test("GET world host-audit-log rejects viewers", async (context) => {
  const viewer = await query(
    `SELECT u.id FROM users u
     ...
     LIMIT 1`,
    [fixtureWorldId, hostUserId, playerUserId]
  );
  if (!viewer.rowCount) {
  ...
  const response = await app.inject({
    method: "GET",
    url: `/api/worlds/${fixtureWorldId}/host-audit-log`,
...
test("GET world host-audit-log aggregates room audit entries", async (context) => {
  ...
  const audit = await app.inject({
    method: "GET",
    url: `/api/worlds/${fixtureWorldId}/host-audit-log?limit=10`,
    headers: { "x-user-id": hostUserId }
  });
  assert.equal(audit.statusCode, 200);
  const { entries } = audit.json();
  assert.ok(Array.isArray(entries));
  const hit = entries.find((row) => row.action === "host_grant_clue" && row.room_id === fixtureRoomId);
