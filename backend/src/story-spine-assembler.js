...
function compactText(parts = [], maxLength = 3000) {
  return rows(parts).map((part) => text(part, maxLength)).filter(Boolean).join("\n").slice(0, maxLength);
}
...
function creatorInput(value = {}) {
  ...
  return {
    ...
    sparks: rows(source.sparks).slice(0, 30).map((item) => ({
      tag: text(item?.tag, 80),
      text: text(item?.text, 800)
    })).filter((item) => item.text),
    sellingPoints: rows(source.sellingPoints).slice(0, 6).map((item) => text(item, 800)).filter(Boolean),
...
export function buildStorySpineSourceCatalog(snapshot = {}, inputValue = {}) {
  ...
  const catalog = [];
  const roleNames = new Map(rows(snapshot.roles).map((role) => [String(role.id), text(role.name, 120)]));

  pushEntry(catalog, sourceEntry("world:summary", "world", "世界简介", input.logline || world.summary));
  pushEntry(catalog, sourceEntry("world:name", "world", "作品名称", world.name));
  pushEntry(catalog, sourceEntry("brief:selling-points", "brief", "核心卖点", compactText(input.sellingPoints.length ? input.sellingPoints : brief.sellingPoints)));
  const target = input.target || brief.target || "";
  const duration = input.duration || brief.duration || "";
  const storyType = input.type || brief.type || "";
  pushEntry(catalog, sourceEntry("brief:positioning", "brief", "创作定位", compactText([
    target ? `目标玩家：${target}` : "",
    duration ? `时长：${duration}` : "",
    storyType ? `类型：${storyType}` : ""
  ])));
  pushEntry(catalog, sourceEntry("brief:sparks", "brief", "灵感卡", compactText(
    (input.sparks.length ? input.sparks : rows(brief.sparks)).map((item) => `${item.tag || "灵感"}：${item.text || ""}`),
    5000
  )));
  pushEntry(catalog, sourceEntry("constitution:theme", "constitution", "核心主题", constitution.theme));
  pushEntry(catalog, sourceEntry("constitution:experience", "constitution", "体验承诺", constitution.experiencePromise));
  pushEntry(catalog, sourceEntry("constitution:principles", "constitution", "不可破坏原则", compactText(constitution.inviolablePrinciples || [])));
  pushEntry(catalog, sourceEntry("constitution:debates", "constitution", "希望玩家争论什么", constitution.desiredDebates));
  pushEntry(catalog, sourceEntry("author:focus", "author", "本次装配重点", input.focus));

  for (const role of rows(snapshot.roles).slice(0, 12)) {
    pushEntry(catalog, sourceEntry(
      `role:${role.id}`,
      "role",
      `角色：${role.name || "未命名角色"}`,
      compactText([
  ...
  pushEntry(catalog, sourceEntry("core-trick:main", "truth", "核心谜底", compactText([
    core.summary,
    core.method,
    core.motive,
  ...
  for (const clue of rows(snapshot.clues).slice(0, 50)) {
    pushEntry(catalog, sourceEntry(
      ...
      "clue",
      `线索：${clue.name || "未命名"}`,
      compactText([clue.public_text, clue.publicText, clue.description], 2500)
    ));
  }
...
function outputContract() {
  return {
    ...
    mechanismLoop: { text: "玩家反复执行的具体行动、反馈与代价", sourceRefs: [] },
    ...
    chapterArc: [{
      chapterId: "优先使用已有 chapter:* 中的 id；未建章节可用 draft-chapter-1",
      sequence: 1,
      title: "章节标题",
      cause: "本章由什么前置结果引起",
...
export function buildStorySpineMessages(snapshot = {}, inputValue = {}) {
  ...
  const currentSpine = normalizeStorySpine(record(snapshot.world?.settings).storySpine);
  return {
    catalog,
    messages: [
      {
...
function filterRefs(refs, allowed) {
  return rows(refs).filter((ref) => allowed.has(ref) || String(ref).startsWith("assumption:"));
}
...
function constrainSourceRefs(spineValue, catalog) {
  ...
  for (const [key] of STORY_SPINE_CORE_SECTIONS) {
    ...
    if (spine[key].status !== "author_confirmed") spine[key].status = "ai_draft";
  }
  for (const collectionKey of ["roleFunctions", "chapterArc", "endingDirections"]) {
...
function assertUsableStorySpine(spine, snapshot) {
  const requiredBlocks = ["logline", "overview", "centralConflict", "playerPremise"];
  const missing = requiredBlocks.filter((key) => !spine[key]?.text);
  if (missing.length || spine.chapterArc.length < 3 || spine.endingDirections.length < 2) {
    throwErr("DEEPSEEK_RESPONSE_INVALID", "AI 没有返回完整的故事主干，请重试。", {
      missing,
  ...
  if (rows(snapshot.roles).length && !spine.roleFunctions.length) {
    throwErr("DEEPSEEK_RESPONSE_INVALID", "AI 没有说明已有角色在整体故事中的作用，请重试。");
  }
}

export async function assembleStorySpine(snapshot = {}, inputValue = {}, {
  requestJson = requestDeepseekJson,
  now = () => new Date().toISOString(),
  requestId = null
} = {}) {
  const { catalog, messages } = buildStorySpineMessages(snapshot, inputValue);
  if (!catalog.some((entry) => entry.type !== "world" || entry.key === "world:summary")) {
    throwErr("BAD_REQUEST", "请先填写一句话梗概、灵感卡、角色或核心事实，再组装故事总览。");
  }
  ...
  preserved.provenance = {
    ...
    generatedAt: now(),
    sourceRevision: snapshot.world?.content_revision == null
      ? null
