...
export async function registerPlayerProgressRoutes(app) {
  app.post("/api/rooms/game/submit", { schema: submitMiniGameSchema }, async (request) => {
    const actorId = requireActor(request);
    const { roomId, answer } = request.body;
    const gameId = request.body.instanceId
      || request.body.instance_id
      || request.body.gameId
      || request.body.game_id;
    requirePlayerRole(await requireRoomRole(actorId, roomId));

    return withRoomIdempotency(roomId, request, "player.mini_game_submit", () => (
  ...
  app.post(
    ...
    async (request) => {
      ...
      const roleSlotId = requirePlayerRole(await requireRoomRole(actorId, roomId));
  ...
  app.post("/api/rooms/:roomId/notebook", { schema: notebookEntrySchema }, async (request, reply) => {
    ...
    const entry = await withRoomIdempotency(roomId, request, "player.notebook_create", () => (
      addPlayerNotebookEntry({
        roomId,
        roleSlotId,
        actorId,
  ...
  app.delete(
    ...
    async (request) => {
      ...
      const roleSlotId = requirePlayerRole(await requireRoomRole(actorId, roomId));
      return withRoomIdempotency(roomId, request, "player.notebook_delete", () => (
