...
import { updateWorldSchema, worldIdParams, createWorldSchema, deleteWorldSchema, addWorldMemberSchema, updateWorldMemberSchema, deleteWorldMemberSchema } from "./schemas.js";
...
export async function registerWorldRoutes(app) {
  ...
  app.get("/api/worlds/:worldId", { schema: { params: worldIdParams } }, async (request, reply) => {
    ...
    const result = await query(
      `SELECT w.id, w.name, w.summary, w.status, w.settings, w.created_at, w.updated_at,
