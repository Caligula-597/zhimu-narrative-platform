...
export const CREATOR_ROOM_FIELDS = `
  id, world_id, host_user_id, name, invite_code, status, settings,
...
export async function insertCreatorRoomGraph(client, {
  ...
  const result = await client.query(
    `WITH created_room AS (
       INSERT INTO rooms
         (world_id, host_user_id, name, invite_code, status, public_listing,
          creation_idempotency_key, creation_request_hash)
       VALUES ($1, $2, $3, $4, 'testing', $5, $6, $7)
    ...
    [worldId, actorId, name, inviteCode, publicListing, idempotencyKey, requestHash]
  );
...
export async function findCreatorRoomByCreationKey({ worldId, actorId, idempotencyKey }, client = null) {
  ...
  const result = await db(
    `SELECT ${CREATOR_ROOM_FIELDS}, creation_request_hash
     FROM rooms
     WHERE world_id = $1
       AND host_user_id = $2
...
export async function listCreatorRoomsForActor({ worldId, actorId }) {
  const result = await query(
    `SELECT world_member.role AS membership_role,
            ...
            room.public_listing, room.created_at, room.host_user_id,
     ...
     GROUP BY world_member.role, room.id
