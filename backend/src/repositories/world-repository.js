...
export async function listWorldMembers(worldId) {
  const result = await query(
    `SELECT wm.user_id, u.email, COALESCE((
       SELECT profile.display_name FROM user_portal_profiles profile
       WHERE profile.user_id = u.id AND profile.portal = 'creator'
     ), u.display_name) AS display_name, wm.role, wm.created_at
...
export async function listWorldTimelineLogs(worldId, { roomId = null, eventType = "", keyword = "", limit = 80 } = {}) {
  const result = await query(
    `SELECT tl.id, tl.room_id, r.name AS room_name, tl.event_type, tl.message, tl.visibility,
            tl.metadata, tl.created_at, COALESCE((
              SELECT profile.display_name FROM user_portal_profiles profile
              WHERE profile.user_id = u.id
                AND profile.portal = CASE
                  WHEN u.id = r.host_user_id THEN 'host'
                  ELSE 'player'
                END
