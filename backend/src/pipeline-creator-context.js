...
function appendSection(base, label, rows, maximum) {
  const content = (Array.isArray(rows) ? rows : [rows]).filter(Boolean).join("\n").trim();
...
function formatRoleFunctions(spine) {
  return spine.roleFunctions.map((role) =>
...
function formatCreatorBrief(brief) {
  const sparks = (Array.isArray(brief.sparks) ? brief.sparks : [])
    .slice(0, 12)
    .map((spark) => `${text(spark?.tag, 80) || "灵感"}：${text(spark?.text, 800)}`)
    .filter((row) => !row.endsWith("："));
  return [
    lines(brief.sellingPoints, 6).length ? `核心卖点：${lines(brief.sellingPoints, 6).join("；")}` : "",
...
function formatConstitution(constitution) {
  return [
    ...
    constitution.experiencePromise ? `体验承诺：${constitution.experiencePromise}` : "",
    ...
    constitution.desiredDebates ? `希望争论：${constitution.desiredDebates}` : "",
    constitution.avoidMisunderstandings ? `避免误解：${constitution.avoidMisunderstandings}` : ""
  ].filter(Boolean);
}
 ...
 */
export function applyCreatorContextToPipelineInput(input = {}, settingsValue = {}) {
  ...
  const constitution = normalizeCreativeConstitution(settings.creativeConstitution);
  ...
  synopsis.body = appendSection(
    ...
    "创作驾驶舱·权威创作上下文",
    [
      formattedBrief.length ? `【产品与体验目标】\n${formattedBrief.join("\n")}` : "",
      formattedSpine.length ? `【作者已确认故事主轴】\n${formattedSpine.join("\n")}` : ""
    ],
    12_000
  );
