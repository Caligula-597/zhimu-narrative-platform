...
当前易变化指标由 [`docs/GENERATED_PROJECT_STATUS.json`](./docs/GENERATED_PROJECT_STATUS.json) 生成；全部文档的生命周期与优先级见 [文档总索引](./docs/DOCUMENTATION_INDEX_ZH.md)。
...
| 应用 | 目录 | 生产域 | 说明 |
|---|---|---|---|
| 主应用 | 根目录 `src/` | `app.getzhimu.com` | 创作者、资产、发布准备、账号与 OPS |
...
已完成的大入口收敛：`world-helpers.js` 为 6 行兼容 barrel，`player-routes.js` 为 8 行注册器，原 2200+ 行 schema 已拆为 32 个领域文件，`play/src/main.js` 当前约 413 行且只负责启动编排。当前 70 个路由模块的直接数据库调用点已经归零，并由 `npm run check:architecture` 固定为不可回升门禁；后续结构审计转向 service/repository 内部查询往返、事务边界和跨领域依赖。
...
- 文档历史版本较多，所有当前标准应从 `docs/DOCUMENTATION_INDEX_ZH.md` 进入；历史证据和方案文档不得覆盖当前事实。
