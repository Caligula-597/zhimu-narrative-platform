...
- 会话校验不再每次请求都写 `last_seen_at`，默认最多每 5 分钟触碰一次；不存在账号也执行等成本的 dummy scrypt 校验，降低登录枚举时序差。
- SSE 服务端增加单连接写缓冲和 replay 队列上限，慢客户端会被主动断开；PostgreSQL LISTEN 冷启动失败会保留订阅并指数退避恢复。
- 投票写链路已迁移到 repository/service；投票关闭与玩家提交在同一行锁事务内串行化，消除“校验时开放、落库时已关闭”的竞态。
- 周期报告使用明确 UTF-8 输出；编码门禁覆盖 735 个生产与审计脚本，并识别常见二次转码乱码。三端 gzip 预算已加入周期验收。
...
本轮快速证据包括：依赖生产审计四端 0 个 high/critical 漏洞、三端构建通过、SSE 故障矩阵 27/27、共享 transport 81/81、LISTEN 冷启动故障注入恢复、外部请求/SSRF/限流/错误脱敏/静态路径/优雅关闭的定向测试，以及包体预算全部通过。无 `DATABASE_URL` 时，真实 PostgreSQL 会话触碰与 LISTEN 集成断言必须明确标记跳过，不能计作通过。
