...
- 投票写链路已迁移到 repository/service；投票关闭与玩家提交在同一行锁事务内串行化，消除“校验时开放、落库时已关闭”的竞态。主持创建投票新增房间级幂等保护，投票、候选项、时间线、SSE outbox 与主持审计在同一事务提交。
- Host 事件处理、投票创建和运行房创建改为固定上下文的页面内事务工作区；重复点击被锁定，断线不确定态复用原幂等键核对，业务提交成功但后续列表刷新失败不会再误报为写入失败。
- 后端 `find-my-way` 已从受 HTTP/2 DDoS 高危公告影响的 9.6.0 升级至 9.7.0；`npm audit --audit-level=high --omit=dev` 当前为 0 漏洞。
...
当前代码结构由 `npm run check:architecture` 固定门禁：全部路由模块的路由层直连数据库点为 0，任何回升都会失败；模块数量以生成基线为准。后续数据库审计对象转为 service/repository 内部的往返次数、连接池占用、索引和事务一致性，而不是继续按文件机械拆层。
