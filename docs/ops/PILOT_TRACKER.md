# 内测试点团队追踪（P1-08）

> 由产品/运营维护。可复制到 Notion/飞书；本文件为仓库内备份。  
> **当前已登记 1 个试点团队**（2026-07-04）。官网 `#case-study` 文案仍为占位，待首场跑通后再同步 site。  
> 决策背景：[DESIGN_DECISIONS_NEEDED_ZH.md](../DESIGN_DECISIONS_NEEDED_ZH.md)

---

| # | 团队/联系人 | 剧本类型 | 导入方式 | 首次开房目标 | 反馈摘要 | 状态 |
|---|-------------|----------|----------|--------------|----------|------|
| 1 | **Caligula** · 3145976907@qq.com | 长线本（自助创作） | **自助** | 完成创建→开房→邀请→跑完一幕 | admin 告警已验证；creator 已开通 | **已 onboarding** |
| 2 | **B0-01 内部** · 停雪公馆 pilot | 6 人本格 · Matrix 2.0 | **import-package** | 首场跑通 ch1→ch3 | 机械 18/18；12 条分幕解锁规则；邀请码 `停雪公馆-5367B7` | **已导入** |
| 3 | | | | | | |
| 4 | | | | | | |
| 5 | | | | | | |

**状态**：待联系 · 已 onboarding · 已开房 · 跑完一局 · 付费意向 · 暂停

**每团队最少记录**：

- [ ] 是否独立完成向导创建测试房  
- [ ] 主持台是否看到玩家进度  
- [ ] 玩家端是否顺畅（邀请码 / 读幕）  
- [ ] 是否愿意第二场  

Support：`support@getzhimu.com` · 流程 [BETA_SUPPORT_SOP_ZH.md](./BETA_SUPPORT_SOP_ZH.md) · 商业试点 [COMMERCIAL_PILOT_SOP_ZH.md](./COMMERCIAL_PILOT_SOP_ZH.md) · 订单 [PILOT_ORDER_LOG.md](./PILOT_ORDER_LOG.md) · Checklist [BETA_ONBOARDING_CHECKLIST_ZH.md](./BETA_ONBOARDING_CHECKLIST_ZH.md)
