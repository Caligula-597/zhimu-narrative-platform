...
复盘和存档点创建统一使用 `host-archive-workspace`，不得重新放回邀请弹窗或创建两个重复表单。model 保存两类独立草稿和 120/2000 字符边界；service 固定使用打开时的 roomId，向复盘、存档端点发送显式幂等键，并在响应不确定时冻结草稿、复用原键核对。

这两个端点后端事务窗口为 30 秒，客户端超时必须大于该窗口，当前为 45 秒。提交成功后历史读取失败或暂时未出现新记录不能覆盖已确认结果，更不能引导用户再次创建。
