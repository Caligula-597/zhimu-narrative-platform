...
function requirementSatisfied(current, operator, expected) {
  if (operator === "equals") return valueSignature(current) === valueSignature(expected);
  if (operator === "not_equals") return valueSignature(current) !== valueSignature(expected);
  if (operator === "includes") return Array.isArray(current)
    ? current.some((entry) => valueSignature(entry) === valueSignature(expected))
...
function validateStateValue(packageValue, stateKey, value) {
  const state = packageValue.stateRegistry.find((entry) => entry.key === stateKey);
  if (!state) fail("MECHANISM_STATE_UNKNOWN", `Unknown mechanism state ${stateKey}`, { stateKey });
  if (state.valueType === "number" && !Number.isFinite(Number(value))) {
    fail("MECHANISM_STATE_INVALID", `State ${stateKey} must be numeric`, { stateKey, value });
  }
  if (state.valueType === "boolean" && typeof value !== "boolean") {
    fail("MECHANISM_STATE_INVALID", `State ${stateKey} must be boolean`, { stateKey, value });
  }
  if (state.valueType === "set" && !Array.isArray(value)) {
    fail("MECHANISM_STATE_INVALID", `State ${stateKey} must be a set array`, { stateKey, value });
  }
  if (state.valueType === "enum" && asArray(state.allowedValues).length
    && !state.allowedValues.some((entry) => valueSignature(entry) === valueSignature(value))) {
    fail("MECHANISM_STATE_INVALID", `State ${stateKey} is outside its allowed values`, { stateKey, value });
  }
}

function validateResourceValue(packageValue, resourceKey, value) {
  const resource = packageValue.resourceRegistry.find((entry) => entry.key === resourceKey);
  if (!resource) fail("MECHANISM_RESOURCE_UNKNOWN", `Unknown mechanism resource ${resourceKey}`, { resourceKey });
  const number = Number(value);
  if (!Number.isFinite(number)) fail("MECHANISM_RESOURCE_INVALID", `Resource ${resourceKey} must be numeric`, { resourceKey, value });
  if (Number.isFinite(resource.minimum) && number < resource.minimum) {
    fail("MECHANISM_RESOURCE_OUT_OF_BOUNDS", `Resource ${resourceKey} fell below its minimum`, {
      resourceKey, value: number, minimum: resource.minimum
    });
  }
  if (Number.isFinite(resource.maximum) && number > resource.maximum) {
    fail("MECHANISM_RESOURCE_OUT_OF_BOUNDS", `Resource ${resourceKey} exceeded its maximum`, {
      resourceKey, value: number, maximum: resource.maximum
    });
  }
}
...
function setChange(changes, targetType, targetKey, before, after, sourceKey) {
  ...
  changes.push({ targetType, targetKey, before: clone(before), after: clone(after), sourceKey });
}
...
function applyEffect(runtimeInput, effect, packageValue, changes, sourceKey) {
  ...
  if (effect?.targetType === "state") {
    ...
    else if (effect.operation === "increment") after = Number(before || 0) + Number(effect.value || 0);
    else if (effect.operation === "decrement") after = Number(before || 0) - Number(effect.value || 0);
    else if (effect.operation === "add") after = [...asArray(before), clone(effect.value)].filter((entry, index, rows) => (
      rows.findIndex((candidate) => valueSignature(candidate) === valueSignature(entry)) === index
    ));
  ...
  if (effect?.targetType === "resource") {
    ...
    let after = before;
    if (effect.operation === "gain") after = current + Number(effect.amount || 0);
    else if (effect.operation === "lose") after = current - Number(effect.amount || 0);
  ...
  if (effect?.targetType === "evidence") {
    if (packageValue && !packageValue.evidenceGraph.evidence.some((entry) => entry.key === targetKey)) {
      fail("MECHANISM_EVIDENCE_UNKNOWN", `Unknown mechanism evidence ${targetKey}`, { targetKey });
    }
  ...
  fail("MECHANISM_EFFECT_INVALID", "Mechanism effect target type is invalid", { effect });
}

function applyStateWrites(runtimeInput, writes, packageValue, changes, sourceKey) {
  let runtime = runtimeInput;
  for (const write of asArray(writes)) runtime = applyEffect(runtime, {
    targetType: "state",
    targetKey: write.stateKey,
    operation: write.operation,
    value: write.value
  }, packageValue, changes, sourceKey);
  return runtime;
}

function applyResourceDeltas(runtimeInput, deltas, packageValue, changes, sourceKey) {
  let runtime = runtimeInput;
  for (const delta of asArray(deltas)) runtime = applyEffect(runtime, {
    targetType: "resource",
...
function applyEvidence(runtimeInput, unlocks, locks, changes, sourceKey) {
  let runtime = runtimeInput;
  for (const key of asArray(unlocks)) runtime = applyEffect(runtime, {
    targetType: "evidence", targetKey: key, operation: "unlock"
  }, null, changes, sourceKey);
  for (const key of asArray(locks)) runtime = applyEffect(runtime, {
    targetType: "evidence", targetKey: key, operation: "lock"
  }, null, changes, sourceKey);
  return runtime;
}

function facts(packageValue) {
  return Object.fromEntries(packageValue.factLedger.map((fact) => [fact.key, fact.truthValue]));
}

function prepareRound(runtimeInput, packageValue, round) {
  if (runtimeInput.preparedRoundKey === round.key) return { runtime: clone(runtimeInput), changes: [] };
  let runtime = clone(runtimeInput);
  const changes = [];
  const readResults = round.stateReads.map((read) => requirementSatisfied(runtime.states[read.stateKey], read.operator, read.value));
  ...
  runtime.currentVariantKey = readPassed ? round.onReadPass?.variantKey || null : round.onReadFail?.variantKey || null;
  if (!readPassed) {
    runtime = applyStateWrites(runtime, round.onReadFail?.stateWrites, packageValue, changes, `${round.key}:read_fail`);
    runtime = applyResourceDeltas(runtime, round.onReadFail?.additionalCosts, packageValue, changes, `${round.key}:read_fail`);
    runtime = applyEvidence(runtime, round.onReadFail?.unlocksEvidenceKeys, round.onReadFail?.locksEvidenceKeys, changes, `${round.key}:read_fail`);
  }
  runtime = applyStateWrites(runtime, round.stateWrites, packageValue, changes, `${round.key}:mandatory`);
  runtime = applyResourceDeltas(runtime, round.resourceDeltas, packageValue, changes, `${round.key}:mandatory`);
  runtime = applyEvidence(runtime, round.unlocksEvidenceKeys, round.locksEvidenceKeys, changes, `${round.key}:mandatory`);
...
function applyWorldRules(runtimeInput, packageValue, roundKey) {
  ...
  for (const rule of packageValue.worldRules.filter((entry) => entry.evaluationChapterKey === roundKey)) {
    ...
    const triggersPass = asArray(rule.triggerEventKeys).every((key) => runtime.events[key] === true);
...
export function initializeMechanismRuntime(packageInput) {
  ...
  const rounds = [...packageValue.rounds].sort((left, right) => left.sequence - right.sequence);
  ...
  for (const state of packageValue.stateRegistry) validateStateValue(packageValue, state.key, runtime.states[state.key]);
  for (const resource of packageValue.resourceRegistry) validateResourceValue(packageValue, resource.key, runtime.resources[resource.key]);
  if (!firstRound) return { runtime, changes: [] };
  return prepareRound(runtime, packageValue, firstRound);
}

export function executeMechanismDecision(runtimeInput, packageInput, { decisionKey, optionKey }) {
  const packageValue = assertMechanismPackage(packageInput);
  if (runtimeInput.status !== "running") fail("MECHANISM_RUNTIME_COMPLETED", "Mechanism runtime is already completed");
  const decision = packageValue.decisionNodes.find((entry) => entry.key === decisionKey && entry.roundKey === runtimeInput.currentRoundKey);
  if (!decision) fail("MECHANISM_DECISION_UNAVAILABLE", "Decision is not available in the current round", { decisionKey });
  if (runtimeInput.decisionStates[decision.key]) fail("MECHANISM_DECISION_ALREADY_RESOLVED", "Decision has already been resolved", { decisionKey });
  const option = decision.options.find((entry) => entry.key === optionKey);
  if (!option) fail("MECHANISM_OPTION_INVALID", "Decision option is invalid", { decisionKey, optionKey });
  let runtime = clone(runtimeInput);
  const changes = [];
...
export function executeMechanismInvestigation(runtimeInput, packageInput, { investigationKey, outcome = "success" }) {
  const packageValue = assertMechanismPackage(packageInput);
  if (runtimeInput.status !== "running") fail("MECHANISM_RUNTIME_COMPLETED", "Mechanism runtime is already completed");
  const action = packageValue.investigationActions.find((entry) => (
    entry.key === investigationKey && entry.roundKey === runtimeInput.currentRoundKey
  ));
  if (!action) fail("MECHANISM_INVESTIGATION_UNAVAILABLE", "Investigation is not available in the current round", { investigationKey });
  if (runtimeInput.executedInvestigations[action.key]) {
    fail("MECHANISM_INVESTIGATION_ALREADY_RESOLVED", "Investigation has already been resolved", { investigationKey });
  }
  const branch = outcome === "success" ? action.success : action.failure;
  if (!branch) fail("MECHANISM_INVESTIGATION_OUTCOME_UNSUPPORTED", "This investigation does not define the requested outcome", { investigationKey, outcome });
  let runtime = clone(runtimeInput);
  const changes = [];
...
export function executeMechanismOverride(runtimeInput, packageInput, { effects, reason }) {
  const packageValue = assertMechanismPackage(packageInput);
  const normalizedReason = String(reason ?? "").trim();
  if (normalizedReason.length < 10) {
    fail("MECHANISM_OVERRIDE_REASON_REQUIRED", "Host overrides require a concrete audit reason");
  }
  if (!Array.isArray(effects) || effects.length === 0) {
    fail("MECHANISM_OVERRIDE_EFFECTS_REQUIRED", "Host overrides require at least one explicit effect");
  }
  let runtime = clone(runtimeInput);
  const changes = [];
  for (const effect of effects) {
    runtime = applyEffect(runtime, effect, packageValue, changes, "host-override");
  }
  if (runtime.status === "completed") runtime.ending = resolveEnding(runtime, packageValue);
  return {
    runtime,
    changes,
    action: { type: "override", overrideKey: "host-override", reason: normalizedReason }
  };
}
...
export function advanceMechanismRound(runtimeInput, packageInput) {
  const packageValue = assertMechanismPackage(packageInput);
  if (runtimeInput.status !== "running") fail("MECHANISM_RUNTIME_COMPLETED", "Mechanism runtime is already completed");
  const currentRound = packageValue.rounds.find((round) => round.key === runtimeInput.currentRoundKey);
  if (!currentRound) fail("MECHANISM_ROUND_UNKNOWN", "Current mechanism round does not exist", { roundKey: runtimeInput.currentRoundKey });
  const unresolved = packageValue.decisionNodes
    .filter((decision) => decision.roundKey === currentRound.key && decision.options.length)
    .filter((decision) => !runtimeInput.decisionStates[decision.key]);
  if (unresolved.length) fail("MECHANISM_DECISIONS_PENDING", "Current round still has unresolved decisions", {
    decisionKeys: unresolved.map((decision) => decision.key)
  });

  const ruleResult = applyWorldRules(runtimeInput, packageValue, currentRound.key);
  let runtime = ruleResult.runtime;
  const changes = [...ruleResult.changes];
  const rounds = [...packageValue.rounds].sort((left, right) => left.sequence - right.sequence);
  const currentIndex = rounds.findIndex((round) => round.key === currentRound.key);
  const nextRound = rounds[currentIndex + 1] ?? null;
  if (!nextRound) {
    runtime.status = "completed";
    runtime.ending = resolveEnding(runtime, packageValue);
    return { runtime, changes, action: { type: "advance", fromRoundKey: currentRound.key, toRoundKey: null } };
  }
  ...
  return {
    ...
    action: { type: "advance", fromRoundKey: currentRound.key, toRoundKey: nextRound.key }
  };
}
...
export function projectMechanismRuntime(runtimeInput, packageInput) {
  const packageValue = assertMechanismPackage(packageInput);
  const runtime = clone(runtimeInput);
  const round = packageValue.rounds.find((entry) => entry.key === runtime.currentRoundKey) ?? null;
  return {
    ...runtime,
    currentRound: round,
    availableDecisions: packageValue.decisionNodes
      .filter((decision) => decision.roundKey === runtime.currentRoundKey && !runtime.decisionStates[decision.key])
      .map(clone),
    availableInvestigations: packageValue.investigationActions
      .filter((action) => action.roundKey === runtime.currentRoundKey && !runtime.executedInvestigations[action.key])
      .map(clone)
  };
}
...
function actionDescriptor(action) {
  if (action.type === "decision") return `decision:${action.decisionKey}:${action.optionKey}`;
  if (action.type === "investigation") return `investigation:${action.investigationKey}:${action.outcome}`;
  return `advance:${action.fromRoundKey || "final"}`;
}

export function analyzeMechanismRuntimeReachability(runtimeInput, packageInput, { pathLimit = 2048 } = {}) {
  const packageValue = assertMechanismPackage(packageInput);
  const safeLimit = Number.isInteger(pathLimit) && pathLimit > 0 ? Math.min(pathLimit, 20_000) : 2048;
  ...
  const enqueue = (entry, task) => {
    try {
      const result = task();
      queue.push({ runtime: result.runtime, trace: [...entry.trace, actionDescriptor(result.action)] });
    } catch (error) {
      const key = `${error?.code || "UNKNOWN"}:${JSON.stringify(error?.details ?? {})}`;
      if (blockedKeys.has(key)) return;
      blockedKeys.add(key);
      blocked.push({ code: error?.code || "MECHANISM_ACTION_BLOCKED", message: error?.message || "Action blocked", details: error?.details });
    }
  };
  ...
  while (queue.length && exploredStateCount < safeLimit) {
    ...
    for (const decision of projection.availableDecisions) {
      for (const option of decision.options) {
        enqueue(entry, () => executeMechanismDecision(entry.runtime, packageValue, {
    ...
    for (const investigation of projection.availableInvestigations) {
      enqueue(entry, () => executeMechanismInvestigation(entry.runtime, packageValue, {
  ...
  const reachableRouteKeys = [...new Set(completed.map((entry) => entry.runtime.ending?.resolvedRouteKey).filter(Boolean))];
  const currentFacts = facts(packageValue);
  const endingProspects = packageValue.endingRoutes.map((route) => {
    const unmetRequirements = asArray(route.requirements).filter((requirement) => !requirementSatisfied(
      collectionFor(runtimeInput, requirement.targetType, currentFacts)?.[requirement.targetKey],
      requirement.operator,
      requirement.value
    )).map((requirement) => ({
      ...
      current: clone(collectionFor(runtimeInput, requirement.targetType, currentFacts)?.[requirement.targetKey])
    }));
    const sample = completed.find((entry) => entry.runtime.ending?.resolvedRouteKey === route.key);
    return {
      key: route.key,
