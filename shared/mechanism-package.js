...
const cleanKey = (value) => String(value ?? "").trim().slice(0, 120);
...
function requireReference(known, key, label) {
  ...
  if (normalized && !known.has(normalized)) throw new TypeError(`${label} references unknown key ${normalized}`);
}

function resourceDeltaReferences(rows, resourceKeys, label) {
  for (const row of asArray(rows)) requireReference(resourceKeys, row?.resourceKey, label);
}

function stateWriteReferences(rows, stateKeys, label) {
  for (const row of asArray(rows)) requireReference(stateKeys, row?.stateKey, label);
}
...
function optionEffects(option, decisionStateKey) {
  ...
  if (legacyStateKey && legacyValue !== undefined && legacyValue !== null && !effects.some((effect) => (
    effect?.targetType === "state" && cleanKey(effect?.targetKey) === legacyStateKey
  ))) {
    effects.push({
      targetType: "state",
...
function compileRound(beat, index) {
  ...
  return {
    ...
    triggerRoleKeys: asArray(beat?.triggerRoleKeys).map(cleanKey).filter(Boolean),
    ...
    entryConditionMode: String(beat?.entryConditionMode ?? "none").trim() || "none",
    ...
    unlocksEvidenceKeys: asArray(beat?.unlocksEvidenceKeys).map(cleanKey).filter(Boolean),
    locksEvidenceKeys: asArray(beat?.locksEvidenceKeys).map(cleanKey).filter(Boolean),
    ...
    decisionKey: cleanKey(beat?.decision?.key || (asArray(beat?.decision?.options).length ? `${chapterKey}-decision` : ""))
  };
}

function compileActions(players) {
  return asArray(players).flatMap((player) => asArray(player?.chapterActions).map((action, index) => ({
    key: cleanKey(action?.key || `${player?.key || "role"}-${action?.chapterKey || index + 1}-action`),
    ...
    stateWriteKeys: asArray(action?.stateWriteKeys).map(cleanKey).filter(Boolean),
    resourceKeys: asArray(action?.resourceKeys).map(cleanKey).filter(Boolean),
    evidenceEffectKeys: asArray(action?.evidenceEffectKeys).map(cleanKey).filter(Boolean),
    evidenceKeys: asArray(action?.evidenceKeys).map(cleanKey).filter(Boolean),
    affectsRoleKeys: asArray(action?.affectsRoleKeys).map(cleanKey).filter(Boolean)
  })));
}
...
function compileInvestigationActions(evidenceRows) {
  return asArray(evidenceRows).map((evidence) => ({
    ...
    operation: String(evidence?.methodOperation ?? evidence?.collectionMethod ?? "").trim(),
    input: {
      originRootKeys: asArray(evidence?.originRootKeys).map(cleanKey).filter(Boolean),
      storageEntityKey: cleanKey(evidence?.storageEntityKey),
      methodDomain: String(evidence?.methodDomain ?? "").trim()
    },
    success: {
      unlocksEvidenceKeys: cleanKey(evidence?.key) ? [cleanKey(evidence.key)] : [],
...
function compileBranchFragments(outline) {
  const eventFragments = asArray(outline?.semanticConstitution?.branchEvents).map((event) => ({
  ...
  const readFragments = asArray(outline?.chapterBeats).flatMap((beat) => {
    ...
    if (cleanKey(pass.variantKey)) result.push({
    ...
    if (cleanKey(fail.variantKey)) result.push({
      ...
      unlocksEvidenceKeys: asArray(fail.unlocksEvidenceKeys).map(cleanKey).filter(Boolean),
      locksEvidenceKeys: asArray(fail.locksEvidenceKeys).map(cleanKey).filter(Boolean)
    });
    return result;
  });
...
function compileRoleDisclosureStates(players) {
  return asArray(players).map((player) => ({
    ...
    secretFactKeys: asArray(player?.secretFactKeys).map(cleanKey).filter(Boolean),
    authorizationGrantKeys: asArray(player?.authorizationGrantKeys).map(cleanKey).filter(Boolean),
    disclosureStateKey: cleanKey(player?.disclosureStateKey)
  }));
}

export function compileMechanismPackage(outline, { source = "outline_import" } = {}) {
  ...
  const packageValue = {
    ...
    authorizationMatrix: clone(asArray(value.semanticConstitution?.authorizationGrants)),
    ...
    endingResolution: {
      ...
      conflictResolution: String(value.endingLogic?.conflictResolution ?? "").trim()
    },
...
export function assertMechanismPackage(packageValue) {
  ...
  if (value.schemaVersion !== MECHANISM_PACKAGE_SCHEMA_VERSION) {
    throw new TypeError(`Unsupported mechanism package schema version ${value.schemaVersion ?? "missing"}`);
  }
  const requiredArrays = [
    "factLedger", "entities", "authorizationMatrix", "eventLedger", "stateRegistry", "resourceRegistry", "rounds",
    "actions", "investigationActions", "decisionNodes", "branchFragments", "endingRoutes",
    "roleDisclosureStates", "worldRules"
  ];
  for (const field of requiredArrays) {
    if (!Array.isArray(value[field])) throw new TypeError(`Mechanism package field ${field} must be an array`);
  }
  if (!value.evidenceGraph || !Array.isArray(value.evidenceGraph.evidence) || !Array.isArray(value.evidenceGraph.conclusions)) {
  ...
  const authorizationKeys = uniqueByKey(value.authorizationMatrix, "authorizationMatrix");
  ...
  const evidenceKeys = uniqueByKey(value.evidenceGraph.evidence, "evidenceGraph.evidence");
  const conclusionKeys = uniqueByKey(value.evidenceGraph.conclusions, "evidenceGraph.conclusions");
  uniqueByKey(asArray(value.evidenceGraph.misdirections), "evidenceGraph.misdirections");
  ...
  for (const disclosure of value.roleDisclosureStates) {
    ...
    if (!roleKey) throw new TypeError("roleDisclosureStates contains an entry without a roleKey");
    if (roleKeys.has(roleKey)) throw new TypeError(`roleDisclosureStates contains duplicate roleKey ${roleKey}`);
    roleKeys.add(roleKey);
    evidenceReferences(disclosure?.secretFactKeys, factKeys, `Role disclosure ${roleKey}`);
    for (const grantKey of asArray(disclosure?.authorizationGrantKeys)) {
      requireReference(authorizationKeys, grantKey, `Role disclosure ${roleKey}`);
    }
  }
  ...
  if (value.endingResolution?.defaultRouteKey) {
    const endingKeys = new Set(value.endingRoutes.map((route) => cleanKey(route.key)));
    requireReference(endingKeys, value.endingResolution.defaultRouteKey, "endingResolution");
  }
  for (const evidence of value.evidenceGraph.evidence) {
    requireReference(roundKeys, evidence?.availableChapterKey, `Evidence ${evidence.key}`);
    ...
    for (const conclusionKey of asArray(evidence?.supportsConclusionKeys)) {
      requireReference(conclusionKeys, conclusionKey, `Evidence ${evidence.key}`);
    }
  }
  for (const conclusion of value.evidenceGraph.conclusions) {
    evidenceReferences(conclusion?.evidenceKeys, evidenceKeys, `Conclusion ${conclusion.key}`);
  }
  ...
  for (const round of value.rounds) {
    ...
    stateWriteReferences(round.onReadFail?.stateWrites, stateKeys, `Round ${round.key} fail branch`);
    resourceDeltaReferences(round.resourceDeltas, resourceKeys, `Round ${round.key}`);
    resourceDeltaReferences(round.onReadFail?.additionalCosts, resourceKeys, `Round ${round.key} fail branch`);
    evidenceReferences(round.evidenceKeys, evidenceKeys, `Round ${round.key}`);
    evidenceReferences(round.unlocksEvidenceKeys, evidenceKeys, `Round ${round.key}`);
    evidenceReferences(round.locksEvidenceKeys, evidenceKeys, `Round ${round.key}`);
    requireReference(decisionKeys, round.decisionKey, `Round ${round.key}`);
  }

  for (const action of value.actions) {
    if (action.roundKey && !roundKeys.has(action.roundKey)) {
      throw new TypeError(`Action ${action.key} references unknown round ${action.roundKey}`);
    }
    requireReference(decisionKeys, action.decisionKey, `Action ${action.key}`);
    evidenceReferences(action.evidenceKeys, evidenceKeys, `Action ${action.key}`);
    evidenceReferences(action.evidenceEffectKeys, evidenceKeys, `Action ${action.key}`);
    for (const stateKey of asArray(action.stateWriteKeys)) requireReference(stateKeys, stateKey, `Action ${action.key}`);
    for (const resourceKey of asArray(action.resourceKeys)) requireReference(resourceKeys, resourceKey, `Action ${action.key}`);
  }
  for (const action of value.investigationActions) {
    if (action.roundKey && !roundKeys.has(action.roundKey)) {
      throw new TypeError(`Investigation ${action.key} references unknown round ${action.roundKey}`);
    }
    if (action.evidenceKey && !evidenceKeys.has(action.evidenceKey)) {
      throw new TypeError(`Investigation ${action.key} references unknown evidence ${action.evidenceKey}`);
    }
  }
  for (const decision of value.decisionNodes) {
    if (!roundKeys.has(decision.roundKey)) throw new TypeError(`Decision ${decision.key} references unknown round ${decision.roundKey}`);
    const optionKeys = new Set();
    for (const option of decision.options) {
      const optionKey = cleanKey(option?.key);
      if (!optionKey || optionKeys.has(optionKey)) throw new TypeError(`Decision ${decision.key} has an invalid or duplicate option key`);
      optionKeys.add(optionKey);
      for (const effect of option.effects) {
        if (effect.targetType === "state" && !stateKeys.has(cleanKey(effect.targetKey))) {
          throw new TypeError(`Decision ${decision.key} references unknown state ${effect.targetKey}`);
        }
        if (effect.targetType === "resource" && !resourceKeys.has(cleanKey(effect.targetKey))) {
          throw new TypeError(`Decision ${decision.key} references unknown resource ${effect.targetKey}`);
        }
        if (effect.targetType === "evidence" && !evidenceKeys.has(cleanKey(effect.targetKey))) {
          throw new TypeError(`Decision ${decision.key} references unknown evidence ${effect.targetKey}`);
        }
      }
    }
  }
  for (const fragment of value.branchFragments) {
    requireReference(roundKeys, fragment.roundKey, `Branch fragment ${fragment.key}`);
    stateWriteReferences(fragment.stateWrites, stateKeys, `Branch fragment ${fragment.key}`);
    resourceDeltaReferences(fragment.resourceDeltas, resourceKeys, `Branch fragment ${fragment.key}`);
    evidenceReferences(fragment.unlocksEvidenceKeys, evidenceKeys, `Branch fragment ${fragment.key}`);
    evidenceReferences(fragment.locksEvidenceKeys, evidenceKeys, `Branch fragment ${fragment.key}`);
  }
  for (const route of value.endingRoutes) {
    for (const requirement of asArray(route?.requirements)) {
      if (requirement?.targetType === "state") requireReference(stateKeys, requirement.targetKey, `Ending ${route.key}`);
      if (requirement?.targetType === "resource") requireReference(resourceKeys, requirement.targetKey, `Ending ${route.key}`);
    }
  }
  return value;
}

export function projectMechanismRound(packageValue, roundKey, { sequence = null } = {}) {
  const value = assertMechanismPackage(packageValue);
  const requestedKey = cleanKey(roundKey);
  const round = value.rounds.find((entry) => entry.key === requestedKey)
    || (Number.isInteger(sequence) ? value.rounds.find((entry) => entry.sequence === sequence) : null);
  if (!round) return null;
  const roundDecisions = value.decisionNodes.filter((entry) => entry.roundKey === round.key);
  const decisionEffects = roundDecisions.flatMap((decision) => decision.options.flatMap((option) => option.effects));
  const stateTargets = new Set([
    ...
    ...asArray(round.onReadFail?.stateWrites).map((entry) => cleanKey(entry?.stateKey)),
    ...decisionEffects.filter((entry) => entry?.targetType === "state").map((entry) => cleanKey(entry?.targetKey))
  ].filter(Boolean));
  const resourceTargets = new Set([
    ...round.resourceDeltas.map((entry) => cleanKey(entry?.resourceKey)),
    ...asArray(round.onReadFail?.additionalCosts).map((entry) => cleanKey(entry?.resourceKey)),
    ...decisionEffects.filter((entry) => entry?.targetType === "resource").map((entry) => cleanKey(entry?.targetKey))
  ].filter(Boolean));
  const endingRouteKeys = value.endingRoutes
    .filter((route) => asArray(route?.requirements).some((requirement) => (
      (requirement?.targetType === "state" && stateTargets.has(cleanKey(requirement?.targetKey)))
      || (requirement?.targetType === "resource" && resourceTargets.has(cleanKey(requirement?.targetKey)))
    )))
  ...
  return {
    ...
    round: clone(round),
    actions: clone(value.actions.filter((entry) => entry.roundKey === round.key)),
    investigationActions: clone(value.investigationActions.filter((entry) => entry.roundKey === round.key)),
    decisionNodes: clone(roundDecisions),
    branchFragments: clone(value.branchFragments.filter((entry) => entry.roundKey === round.key)),
    evidence: clone(value.evidenceGraph.evidence.filter((entry) => round.evidenceKeys.includes(cleanKey(entry?.key)))),
    endingRouteKeys
  };
}
