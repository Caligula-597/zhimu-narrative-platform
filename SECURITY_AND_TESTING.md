...
本页描述当前门禁结构，不手工维护会漂移的绝对测试总数。源文件、迁移、路由和各端测试声明数量见 [`docs/GENERATED_PROJECT_STATUS.json`](./docs/GENERATED_PROJECT_STATUS.json)；“声明存在”不等于“本次已经执行通过”。
...
| `npm run check:docs` | 全部 Markdown 编码、标题、相对链接、索引和生成基线 |
...
| `npm run audit:periodic` | 15 项快速文档、非功能、契约、架构和前端预算门禁，并保存日志 |
| `npm run test:sse-matrix` | SSE 解析、replay、受众、游标与生命周期矩阵 |
| `npm run test:auth-matrix` | 三端登录故障矩阵 |
| `npm run test:trusted-types` | 安全 HTML sink、CSP 与 Trusted Types |
| `npm run test:release-gates` | 重复验证/恢复证据工具防假通过 |
...
- `npm run status:generate` 更新易变化的源代码数量。
- `npm run docs:index` 将所有跟踪的 Markdown 归类为当前事实、产品说明、方案、运维、历史或法务材料。
- `npm run check:docs` 验证相对链接和生成基线，避免发布说明继续引用已删除文件或旧迁移号。
- 带日期的演练报告只证明当时那次运行；新的代码提交必须产生新的证据，不能回写旧报告。
