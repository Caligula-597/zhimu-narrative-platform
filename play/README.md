...
- **公开大厅**（`public_listing` 房间列表 + 剧本封面缩略图）
- 分幕阅读、高亮、笔记本 API
- 探索场景与调查点
- 线索列表、已读、私享
- 背包展示
...
- **Tab 未读提醒**：SSE 触发 `tabPulse` / `tabPulseCount`，切换 Tab 清除
...
1. **首页** — 优先入口：官方示例 / 邀请码 / 找人一起玩（公开大厅）；流程说明折叠在 `<details>`
2. **公开大厅** — `GET /api/platform/public-rooms`；有封面时显示 `worldCoverUrl`
3. **加入向导** — 三步进度条（邀请码 → 选角色 → 进入），展示世界/房间信息与可选席位
4. **房间内概览** — 当前场景、分幕/线索/背包统计、「继续阅读」建议下一步
5. **分幕 / 探索 / 线索 / 背包 / 复盘** — 完整玩家功能，侧边栏显示房间成员
