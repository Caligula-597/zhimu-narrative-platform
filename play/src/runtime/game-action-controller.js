export async function handlePlayGameAction({
  ...
  switch (action) {
    ...
    case "submit-satisfaction":
      ...
      return true;
    case "save-suspicion":
      await saveSuspicion({ button, state, api, render, setToast, formatApiError, pullRoomData });
      return true;
    case "submit-vote-ballot":
      await submitVote({ button, state, api, render, setToast, formatApiError, pullRoomData });
      return true;
    case "submit-private-action":
      await submitPrivateAction({ state, api, render, setToast, formatApiError, pullRoomData, documentRef });
      return true;
    case "read-clue":
...
async function saveSuspicion({ button, state, api, render, setToast, formatApiError, pullRoomData }) {
...
async function submitVote({ button, state, api, render, setToast, formatApiError, pullRoomData }) {
...
async function submitPrivateAction({ state, api, render, setToast, formatApiError, pullRoomData, documentRef }) {
  ...
  const actionType = documentRef.querySelector("[data-private-action-type]")?.value || "ask_host";
  if (!title) {
