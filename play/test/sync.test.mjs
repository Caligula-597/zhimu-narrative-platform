...
test("platform-events has poll fallback when SSE down", () => {
  ...
  assert.match(source, /createPortalEventLifecycle/);
  assert.match(source, /pollMs:\s*PORTAL_POLL_INTERVAL_MS\.platform/);
  assert.match(source, /onInGameCommRefresh/);
  assert.match(source, /refresh:\s*\(\) => runPlatformPoll/);
});
